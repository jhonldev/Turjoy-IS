...
@section('content')
    @if ($validRows || $invalidRows || $duplicatedRows)
        ...
        <div class="w-full text-center text-green-custom-100 font-semibold mt-4" id="file-uploaded-message">Archivo cargado
        </div>
        <div class="flex p-6 space-y-4 md:space-y-6 sm:p-11">

            <div class="w-2/3 relative overflow-x-auto rounded-lg overflow-y-auto "
                style="max-height: 600px; overflow-y: auto;">

                @vite('resources/css/app.css')
                <table class="w-full text-sm text-left ">
                    <thead
                        class="border-b-2 uppercase bg-grey-custom-darkSmoke text-grey-custom-dark sticky top-0 border-grey-custom-light"
                        style="background-color: rgba(51, 51, 51, 0.19);">
                        <tr>
                        ...
                        @if (count($validRows) > 0)
                            @foreach ($validRows as $validRow)
                                <tr class="border-b-2 border-grey-custom-light bg-green-custom-validate ">
                                    <td
                                        class="px-6 py-3 text-grey-custom-dark border-r-2 border-grey-custom-light whitespace-nowrap">
                                        {{ $validRow[0] }}
                                    </td>
                                    <td
                                        class="px-6 py-3 text-grey-custom-dark border-r-2 border-grey-custom-light whitespace-nowrap">
            ...
            <div class="w-1/3 m1-4">
                ...
                <div class="bg-yellow-custom-clone p-4 ml-11 m-3 rounded-lg">
                    <p class="text-grey-custom-dark text-center">
                        No se pudieron cargar debido a que ya existen anteriormente. El primer registro correcto entre
                        Origen y Destino se considera válido, el resto incorrectos.</p>
                </div>
            </div>
        </div>
    @else
        ...
        <div class="flex items-center justify-center w-full">
            <form class="flex flex-col items-center w-full mt-44" action="{{ route('routes.check') }}" method="post"
